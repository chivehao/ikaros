...
- 本轮复验（2026-09-10）：search 模块全量回归 8/8 通过，覆盖关键词/游标、类型/标签筛选、实时授权、事件投影、重建切换和失败重试；`Search.vue` 的查询、重建、失败项重试均调用真实 API，Console 构建通过。
...
- 本轮复验（2026-09-10）：新增 `DefaultScanRunServiceTest` 3/3，覆盖扫描任务提交与审计、禁用来源拒绝、负数 checkpoint 拒绝；ingestion 模块全量回归 13/13 通过。扫描仍受真实 Provider/文件系统联调与 Testcontainers 环境限制，未将单元测试当作集成证据。
...
- 本轮复验（2026-09-10）：Operations 目标测试共 15/15 通过，其中通知 6/6、健康 3/3、审计查询 3/3、后台任务 3/3；通知页与账户偏好页继续使用真实 `/communications/*` API。GitHub 评论/关闭待认证恢复后同步。
...
- 本轮复验（2026-09-10）：reading 全量回归 18/18 通过，其中 EPUB 导入 2/2、目录解析 3/3、书籍信息 1/1；`EbookImport.vue` 已核对真实导入、解析/重试、章节和信息 API，失败状态直接来自服务端。真实 EPUB 文件联调仍需外部样本/Testcontainers。
## B05 Console 对接逐项审计
...
## B10-04 展示缩略图生成失败状态
- 日期：2026-09-10
- 实现：新增 `GET /photos/{id}/thumbnail/status`，从持久化 Background Task 返回缩略图任务状态和失败结果；不存在任务返回 `NOT_REQUESTED`，失败不会伪装成已有缩略图。
- Console：Photo 详情加载时同步读取任务状态，持久化展示 `FAILED` 错误告警、任务 ID 和当前状态；生成任务完成后重新加载资产列表。
...
## C02 房间成员管理（父 issue）
- 整体验收：C02-01 至 C02-06 已按顺序完成，覆盖创建房间、邀请加入、角色分配、成员移除、离开和关闭房间。
- Console 对接：`RoomManager.vue` 接入真实 Room、成员和邀请 API，所有变更后刷新服务端状态；权限、状态和错误由服务端作为最终判断。
- 验证证据：sharing 模块 Maven 构建、Console typecheck/build、运行页 HTTP 200；主要提交：`6feb874d`。
...
## C03-03 同步跳转进度
- 日期：2026-09-10
- 实现：跳转进度通过同一 Room control command 传递，使用状态版本避免覆盖并发控制。
- Console：WatchRoom 提供跳转控制参数，提交后按事件 sequence 回放最新播放位置和状态版本。
- 验证：跳转命令、事件回放和失败反馈已复验；主要提交：既有实现（本 issue 无新增代码）。

## C03-04 限制控制权限
- 日期：2026-09-10
- 实现：Room 控制服务端按房主/成员角色判定播放控制权限，客户端不能通过修改请求字段扩大权限。
...
## B11 相册管理（父 issue）
- 整体验收：B11-01 至 B11-05 已按顺序完成，覆盖相册创建/编辑、照片添加/移除、成员排序、封面设置和相册删除；所有能力均已接入照片管理 Console 页面。
...
## B15-06 阻止目录循环移动
- 日期：2026-09-10
- 实现：持久化 Drive 移动前沿目标父节点向上递归检查，阻止移动到自身或任一子目录，并返回稳定 Conflict 语义。
- Console：移动失败提示覆盖目录循环、名称冲突和节点版本变化，避免用户误以为移动成功。
- 验证：drive 模块测试 7/7；Console `pnpm typecheck`；主要提交：`517d2464`。
...
## B16-06 处理原目录不存在的恢复情况
- 日期：2026-09-10
- 实现：恢复时检查原父目录是否仍为当前 Drive Space 内的活动目录；不可用时自动回退到根目录，节点身份、名称和内容版本保持不变。
- Console：回收站恢复继续复用真实恢复 API；恢复后的节点由真实列表刷新展示，名称冲突仍返回 409 并提示处理。
- 验证：drive 模块测试 8/8（含原父目录不可用回退根目录）；Console `pnpm typecheck`；主要提交：`0ae8d106`。

## B16 文件版本与回收站（父 issue）
- 整体验收：B16-01 至 B16-06 已按顺序完成，覆盖新版本、历史版本、历史恢复、移入回收站、文件恢复及原目录失效回退。
- Console 对接：文件详情页真实接入节点查询、新版本上传、历史版本展示/恢复和移入回收站；回收站页真实接入带版本校验的恢复。
- 验证证据：drive 模块测试 8/8；Console `pnpm typecheck`；主要提交：`05b7b7c1`、`383047dc`、`020942de`、`36cbf988`、`33720f9b`、`0ae8d106`。
...
## B21-04 同步跨端消费进度
- 日期：2026-09-10
- 实现：媒体播放历史和阅读历史继续由服务端会话/进度 API 作为共享真相；播放和阅读页面已有 If-Match 版本控制及恢复位置逻辑。
- Console：工作台“进度”页改为并行读取 `/api/media/playback/history` 与 `/api/reading/history`，合并展示媒体/阅读进度；桌面端表格与移动端卡片均使用服务端数据，接口单独失败不会阻断另一类进度展示。
- 验证：Console `pnpm typecheck`；主要提交：`36a1af30`。

## B21-05 展示断网和重连状态
- 日期：2026-09-10
- 实现：Console 布局监听浏览器 `online`/`offline` 事件，断网时明确提示当前不可同步；恢复连接时提示可以刷新并继续同步。
- Console：状态提示位于全局主内容区，对固定头部和滚动布局都生效；离线提示不伪造服务端成功，已保存的本地离线内容仍按 B20 逻辑读取。
- 验证：Console `pnpm typecheck`；主要提交：`da8dea3`。

## B21 跨端客户端基础能力（父 issue）
- 整体验收：B21-01 至 B21-05 已按顺序完成，覆盖移动端登录/资源浏览、桌面文件上传、移动内容消费、跨端进度汇总和断网/重连状态。
- Console 对接：各页面均复用真实认证、Resource、Attachment、播放、阅读、离线下载和历史进度 API；移动端提供卡片/响应式消费界面，未新增静态假数据接口。
- 验证证据：Console `pnpm typecheck`；主要提交：`af514acb`、`48e8508f`、`36a1af30`、`da8dea3`。
