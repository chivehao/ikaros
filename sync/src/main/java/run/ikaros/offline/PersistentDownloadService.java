...
public class PersistentDownloadService implements DownloadService {
    ...
    private boolean allowed(DownloadState from,DownloadState to){if(from==DownloadState.REMOVED||from==DownloadState.CANCELLED||from==DownloadState.COMPLETED)return false;if(to==DownloadState.COMPLETED&&from!=DownloadState.VERIFYING)return false;return true;}
